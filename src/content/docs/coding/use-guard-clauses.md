...
description: Reduces nesting and complexity by returning early on failure conditions so the happy path remains flat. Use when writing or reviewing functions with multiple nested conditionals. Do not use when early returns would require duplicating cleanup logic that is better expressed with a single exit point.
skill-metadata:
  type: indicator
  keywords: "clean-code, complexity, readability, structure, early-return, guard-clause, nesting, indentation, if"
---

...
description: Enforces structured JSON logging with context fields such as TraceID for backend observability. Use when adding or reviewing log statements in backend services. Do not use for development-only debug scripts or CLI tools where human-readable output is preferred.
skill-metadata:
  type: indicator
  keywords: "backend, logging, observability, json, trace-id"
---

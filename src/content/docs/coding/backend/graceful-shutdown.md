...
description: Ensures backend servers handle SIGTERM and other termination signals by draining active requests before exiting. Use when implementing or reviewing HTTP server startup/shutdown logic. Do not use for short-lived CLI tools or scripts that do not bind to a port.
skill-metadata:
  type: indicator
  keywords: "backend, servers, reliability, graceful-shutdown, signals"
---

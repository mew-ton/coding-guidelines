---
name: hoist-side-effects-to-parents
description: "Enforces hoisting side effects (API calls, routing, state mutations) to Container or Page components to keep leaf components pure. Use when writing or reviewing components that mix UI rendering with data fetching or state mutation. Do not use for form components where local input state is inherently co-located with the UI."
metadata:
  type: "indicator"
  keywords: "sideeffect, container, page, api, routing, architecture, responsibility, separation"
---

---
name: replace-enum-switching-with-strategy-pattern
description: "Refactors switch-on-enum or switch-on-string-union logic into the Strategy Pattern to comply with the Open/Closed Principle. Use when a switch statement grows with new cases or when adding a new variant requires editing multiple switch blocks. Do not use for simple one-off branching that will never grow or that is clearer as a switch."
metadata:
  type: "indicator"
  keywords: "design-pattern, strategy, enum, switch, ocp, refactoring, string-literal, union-type"
---

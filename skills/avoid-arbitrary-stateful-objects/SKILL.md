---
name: avoid-arbitrary-stateful-objects
description: "Prohibits ad-hoc mutable stateful objects and enforces stateless functions or established patterns (Factory/Builder) when state is unavoidable. Use when designing data structures or reviewing code that introduces new class instances or mutable singletons. Do not use to prohibit all state — only uncontrolled, arbitrarily created mutable objects."
metadata:
  type: "constraint"
  keywords: "typescript, state, mutability, functional, singleton, side-effects"
---
